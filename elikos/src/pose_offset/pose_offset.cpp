...
bool vo_valid = false;
geometry_msgs::PosePtr local_pose;
geometry_msgs::Pose local_pose_offset;
geometry_msgs::PoseStamped pose_with_offset;
geometry_msgs::PoseWithCovarianceStampedPtr vo_pose;
ros::Time last_vo;
ros::Time last_local_pos;

const ros::Duration timeout(0.5);

void local_pos_callback(const geometry_msgs::PosePtr pose){
    if (!local_pos_valid) {
...
int main(int argc, char **argv) {
    ...
    nh.subscribe("mavros/local_position/local", 1, &local_pos_callback);
    nh.subscribe("svo/pose", 1, &vo_pose_callback);
    ...
    while(ros::ok()){
        ...
        if(ros::Time::now() - last_vo > timeout && local_pos_valid){
            ...
            vo_valid = false;
            local_pose_offset.position = local_pose->position;
            local_pose_offset.orientation = local_pose->orientation;
        }
