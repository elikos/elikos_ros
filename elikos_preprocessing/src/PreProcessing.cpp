...
PreProcessing::PreProcessing()
{
    ...
    cv::namedWindow("preProcessed", CV_WINDOW_AUTOSIZE);
    cv::namedWindow("B&W preProcessed", CV_WINDOW_AUTOSIZE);
    cv::moveWindow("B&W preProcessed", 100, 0);
}

void PreProcessing::preProcessImage(const cv::Mat& raw, cv::Mat& preProcessed, cv::Mat& bwPreProcessed) const
{
    ...
    cv::Mat thresholded;
    cv::Mat bluredConverted;
    cv::cvtColor(blured, bluredConverted, CV_BGR2GRAY);
    cv::adaptiveThreshold(bluredConverted, thresholded, 200, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, 551, 2);

    bwPreProcessed = thresholded;

    cv::imshow("preProcessed", preProcessed);
    cv::imshow("B&W preProcessed", bwPreProcessed);
    cv::waitKey(1);
}
