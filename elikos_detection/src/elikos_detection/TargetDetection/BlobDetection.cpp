...
void BlobDetection::detectCircles(const cv::Mat& input, cv::Mat& output_w, cv::Mat& output_r, cv::Mat& output_g, cv::Mat& output)
{
    ...
    cv::putText(output, "X: " + std::to_string(sTrans.getOrigin().x()), cv::Point(10, 50), cv::FONT_HERSHEY_PLAIN, 1.0, CV_RGB(255, 0, 0), 2.0);
    cv::putText(output, "Y: " + std::to_string(sTrans.getOrigin().y()), cv::Point(10, 65), cv::FONT_HERSHEY_PLAIN, 1.0, CV_RGB(0, 255, 0), 2.0);
    cv::putText(output, "Z: " + std::to_string(sTrans.getOrigin().z()), cv::Point(10, 80), cv::FONT_HERSHEY_PLAIN, 1.0, CV_RGB(0, 0, 255), 2.0);
    cv::putText(output, "MIN / MAX: " + std::to_string(MIN_CIRCLE_RADIUS) + " / " + std::to_string(MAX_CIRCLE_RADIUS), cv::Point(10, 95), cv::FONT_HERSHEY_PLAIN, 1.0, CV_RGB(0, 0, 255), 2.0);

    double droneHeight = sTrans.getOrigin().z() - 0.14; //0.14 =~ distance lorsque le drone est au sol.

    cv::GaussianBlur(src_gray, src_gray, Size(9, 9), 2, 2);
    /// Apply the Hough Transform to find the circles
    cv::HoughCircles(src_gray, circles, CV_HOUGH_GRADIENT, 1, src_gray.rows / 8, 200, 70, MIN_CIRCLE_RADIUS, MAX_CIRCLE_RADIUS * (1.0 - droneHeight));
...
Eigen::Matrix4f BlobDetection::getPerspectiveProjectionTransform(double focalLength, double width, double height)
{
...
void BlobDetection::removePerspective(const cv::Mat& input, cv::Mat& rectified)
{
    ...
    Eigen::Matrix3f r = (Eigen::AngleAxisf(-pitch, Eigen::Vector3f::UnitX()) * Eigen::AngleAxisf(-roll, Eigen::Vector3f::UnitY()))
                            .toRotationMatrix();
